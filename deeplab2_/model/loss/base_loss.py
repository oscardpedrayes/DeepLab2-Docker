...
class TopKCrossEntropyLossWeights(tf.keras.losses.Loss):
  """This class contains code for top-k cross-entropy with class weighting."""
  ...
  def __init__(self,
    ...
    Raises:
      ValueError: An error occurs when top_k_percent_pixels is not between 0.0
        and 1.0. / number of weights.
    """
    # Implicit reduction might mess with tf.distribute.Strategy, hence we
    # explicitly reduce the loss.
    super(TopKCrossEntropyLossWeights,
          self).__init__(reduction=tf.keras.losses.Reduction.NONE)

    _ensure_topk_value_is_percentage(top_k_percent_pixels)
    if len(weights) is not num_classes:
      raise ValueError('The number of WEIGHTS is not equal to the number of classes. Number of weights: {:d}, number of classes: {:d}'.format(len(weights), num_classes))
  ...
  def call(self, y_true: Dict[Text, tf.Tensor],
    ...
    # Custom loss multiplication
    class_weights = tf.convert_to_tensor(np.array(self._weights), dtype=tf.float32)

    gt = tf.math.argmax(tf.cast(gt, tf.int32), axis=3)
    all_weights = tf.ragged.map_flat_values(lambda x: tf.gather(class_weights, x), gt) 
    #all_weights2 = tf.where(tf.math.argmax(gt, axis=3) == 1, 100.0, 0.1)
    #all_weights = tf.where(gt > -1, gt*self._weights[gt] ,0  )
    #all_weights = tf.where (gt
    #all_weights2 = tf.math.argmax(all_weights, axis=3)
    #all_weights2 = tf.cast(all_weights2, tf.float32)
    ...
    weighted_pixel_losses = tf.multiply(pixel_losses, all_weights)
