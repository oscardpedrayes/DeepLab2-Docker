...
_NUM_SHARDS = 10
_IMAGE_FOLDER_NAME = 'images' # IMAGE FOLDER
_PANOPTIC_MAP_FOLDER_NAME = 'masks' # MASK FOLDER
...
def _get_image_info_from_path(image_path: str) -> Tuple[str, str]:
  ...
  return image_id #sequence_id, image_id
...
def _create_panoptic_tfexample(image_path: str,
  ...
  frame_id = _get_image_info_from_path(image_path)
  ...
  return data_utils.create_tfexample(
    image_data,
    image_format,
    image_name,
...
def _convert_dataset(step_root: str,
  ...
  #is_testing = 'sunny' in dataset_split or 'cloudy' in dataset_split
  is_testing = 'nogt' in dataset_split 
  ...
  for shard_id, image_list in enumerate(image_files_per_shard):
    ...
    with tf.io.TFRecordWriter(output_filename) as tfrecord_writer:
      ...
      for image_path in image_list:
        ...
        image_id = _get_image_info_from_path(image_path)

        #panoptic_map_path = os.path.join(step_root, _PANOPTIC_MAP_FOLDER_NAME, dataset_split, sequence_id,'%s.%s' % (image_id, _LABEL_MAP_FORMAT))
        panoptic_map_path = os.path.join(step_root, _PANOPTIC_MAP_FOLDER_NAME, dataset_split,'%s.%s' % (image_id, _LABEL_MAP_FORMAT))
...
def main(argv: Sequence[str]) -> None:
  ...
  for dataset_split in ('train', 'test'): #('v2opacityaviles','train','test','sunny','cloudy', 'opacityaviles'):
