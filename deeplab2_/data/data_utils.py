...
class SegmentationDecoder(object):
  ...
  def __init__(self,
    ...
    self._is_panoptic_dataset = is_panoptic_dataset # check! for seg 
  ...
  def _decode_label(self, parsed_tensors, label_key):
    ...
    if self._is_panoptic_dataset:
      flattened_label = tf.io.decode_raw(
          parsed_tensors[label_key], out_type=tf.int32) #before int32 
    ...
    label = tf.io.decode_image(parsed_tensors[label_key], channels=1) #check 
    label.set_shape([None, None, 1])
    #import sys
    #tf.print(label, output_stream=sys.stderr) # added 
    return label
