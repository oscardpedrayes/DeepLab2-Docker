...
class Evaluator(orbit.StandardEvaluator):
  ...
  def __init__(self, config, model, loss, global_step, model_dir):
    ...
    # Create Precision and Recall for each class
    self._eval_precision_metrics= []
    self._eval_recall_metrics= []
    for class_i in range(self._dataset_info.num_classes):    
      self._eval_precision_metrics.append(tf.keras.metrics.Precision(
      name=str(class_i) + "_Precision"))
      self._eval_recall_metrics.append(tf.keras.metrics.Recall(
      name=str(class_i) + "_Recall"))
  ...
  def _reset(self):
    ...
    for class_i in range(self._dataset_info.num_classes):    
      self._eval_precision_metrics[class_i].reset_states()
      self._eval_recall_metrics[class_i].reset_states()
  ...
  def _eval_step(self, inputs):
    ...
    if self._decode_groundtruth_label:
      ...
      for class_i in range(self._dataset_info.num_classes):    
        self._eval_precision_metrics[class_i].update_state(
            tf.where(
                tf.equal(inputs[common.GT_SEMANTIC_RAW], class_i), 1,
                0),
            tf.where(
                tf.equal(outputs[common.PRED_SEMANTIC_KEY], class_i), 1,
                0))
                
        self._eval_recall_metrics[class_i].update_state(
            tf.where(
                tf.equal(inputs[common.GT_SEMANTIC_RAW], class_i), 1,
                0),
            tf.where(
  ...
  def eval_end(self, state=None):
    ...
    for class_i in range(self._dataset_info.num_classes):    
      eval_logs['evaluation/precision/' + self._eval_precision_metrics[class_i].name] = (
          self._eval_precision_metrics[class_i].result())
      eval_logs['evaluation/recall/' + self._eval_recall_metrics[class_i].name] = (
          self._eval_recall_metrics[class_i].result())
