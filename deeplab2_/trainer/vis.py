...
_MOTS_TRAIN_ID_TO_EVAL_ID = (
    0, 1, 2, 3, 4, 5, 6
)

_COCO_TRAIN_ID_TO_EVAL_ID = coco_constants.get_id_mapping_inverse()

_EMID_AVI_TRAIN_ID_TO_EVAL_ID = (0, 1, 2, 3, 4, 5) 
...
def _convert_train_id_to_eval_id(
  ...
  elif 'motchallenge' in dataset_name:
    train_id_to_eval_id = _MOTS_TRAIN_ID_TO_EVAL_ID
  elif 'emid_avi' in dataset_name:
    train_id_to_eval_id = _EMID_AVI_TRAIN_ID_TO_EVAL_ID
  else:
    raise ValueError(
...
def store_predictions(predictions: Dict[str, Any], inputs: Dict[str, Any],
  ...
  #print(type(inputs[common.GT_SEMANTIC_RAW]))
  #print(inputs[common.GT_SEMANTIC_RAW][inputs[common.GT_SEMANTIC_RAW]!=0])
  ...
  if common.PRED_PANOPTIC_KEY in predictions:
    ...
    vis_utils.save_parsing_result(
        ...
        colormap_name=colormap_name)
    #maximumval = tf.constant([7])
    #inputs[common.GT_PANOPTIC_RAW] = tf.math.minimum(inputs[common.GT_PANOPTIC_RAW], maximumval)   
    vis_utils.save_parsing_result(
